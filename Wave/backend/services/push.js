...
// Sends to one specific user's own subscribed device(s) — used for things
// like "your deposit was just approved," not admin-wide alerts.
async function notifyUserPush(userId,payload){if(!configured())return {sent:0,disabled:true};setup();const rows=await queryAll("SELECT id, subscription_json FROM push_subscriptions WHERE user_id=?",[userId]);let sent=0;for(const row of rows){try{await webpush.sendNotification(JSON.parse(row.subscription_json),JSON.stringify(payload));sent++;}catch(err){if(err.statusCode===404||err.statusCode===410)await execute("DELETE FROM push_subscriptions WHERE id=?",[row.id]);}}return {sent};}
module.exports={configured,notifyAdmins,notifyUserPush};
