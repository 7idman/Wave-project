...
async function applyRequest(r,admin,action,note){if(r.status!=="pending")throw new Error("Request already reviewed");if(action==="approve"){if(r.type==="deposit"){const amt=Number(r.amount||0);const referenceId=generateReferenceId("DEP");await execute("UPDATE users SET cash_balance=cash_balance+?,updated_at=datetime('now') WHERE id=?",[amt,r.user_id]);const inserted=await execute("INSERT INTO transactions (user_id,type,symbol,amount,price,fee,total,status,reference_id) VALUES (?,?,?,?,?,?,?,'completed',?)",[r.user_id,"deposit","USD",amt,1,0,amt,referenceId]);try{const updated=await queryOne("SELECT cash_balance FROM users WHERE id=?",[r.user_id]);await notifyUserPush(r.user_id,{type:"balance_update",title:"Deposit confirmed",body:`$${amt.toLocaleString()} was added to your balance`,cashBalance:updated.cash_balance});}catch(pushErr){console.error("Deposit approved but balance push failed:",pushErr.message);}try{await applyDepositBonus(r.user_id,amt,inserted.lastInsertRowid);}catch(bonusErr){console.error("Deposit bonus check failed (deposit itself still succeeded):",bonusErr.message);}try{await checkReferralBonus(r.user_id);}catch(refErr){console.error("Referral bonus check failed (deposit itself still succeeded):",refErr.message);}}else if(r.type==="withdraw"){const p=parse(r.payload);const txn=await queryOne("SELECT * FROM transactions WHERE id=? AND status='awaiting_review'",[p.transactionId]);if(!txn)throw new Error("This withdrawal is no longer awaiting review — it may have already been processed.");
    // Same atomic balance+locked-bonus guarded UPDATE used everywhere else
    // money leaves an account — re-checked here, not assumed safe just
    // because the user passed SMS+email verification earlier, since the
    // balance can have moved in the time it took an admin to review this.
    const result=await execute(`UPDATE users SET cash_balance=cash_balance-?,updated_at=datetime('now') WHERE id=? AND cash_balance>=? AND (cash_balance-(SELECT COALESCE(SUM(amount),0) FROM bonus_grants WHERE user_id=? AND unlock_at>datetime('now'))-(SELECT COALESCE(SUM(amount),0) FROM referral_bonus_grants WHERE user_id=? AND unlock_at>datetime('now')))>=?`,[p.total,r.user_id,p.total,r.user_id,r.user_id,p.total]);if(result.rowsAffected===0){await execute("UPDATE transactions SET status='failed' WHERE id=?",[p.transactionId]);throw new Error("Cannot approve — this user no longer has sufficient available balance for this withdrawal.");}await execute("UPDATE transactions SET status='completed' WHERE id=?",[p.transactionId]);try{const updated=await queryOne("SELECT cash_balance FROM users WHERE id=?",[r.user_id]);await notifyUserPush(r.user_id,{type:"balance_update",title:"Withdrawal approved",body:`$${Number(p.amount).toLocaleString()} withdrawal approved`,cashBalance:updated.cash_balance});}catch(pushErr){console.error("Withdrawal approved but balance push failed:",pushErr.message);}}else if(r.type==="kyc"){const p=parse(r.payload);if(p.kind==="id")await execute("UPDATE users SET kyc_id_status='verified',updated_at=datetime('now') WHERE id=?",[r.user_id]);if(p.kind==="address")await execute("UPDATE users SET kyc_addr_status='verified',updated_at=datetime('now') WHERE id=?",[r.user_id]);}else if(r.type==="account_deletion"){await execute("DELETE FROM refresh_tokens WHERE user_id=?",[r.user_id]);await execute("DELETE FROM sessions WHERE user_id=?",[r.user_id]);await execute("DELETE FROM push_subscriptions WHERE user_id=?",[r.user_id]);await execute("DELETE FROM users WHERE id=?",[r.user_id]);}}if(action==="reject"){if(r.type==="withdraw"){const p=parse(r.payload);await execute("UPDATE transactions SET status='rejected' WHERE id=? AND status='awaiting_review'",[p.transactionId]);}await notifyUser(r.user_id,"request_rejected",`Your ${r.type.replace(/_/g," ")} request was rejected${note?`: ${note}`:""}`);}await execute("UPDATE admin_requests SET status=?,reviewed_by=?,reviewed_by_email=?,reviewed_at=datetime('now'),admin_note=? WHERE id=?",[action==="approve"?"approved":"rejected",admin.id,admin.email,note||null,r.id]);}
