...
const request = async (path: string, opts: RequestInit = {}): Promise<any> => {
  ...
  let response = await fetch(`${API_BASE}${path}`, { ...opts, headers });
  if (response.status === 401 && refreshToken) {
    ...
    if (refreshed.ok) { const data=await refreshed.json(); accessToken=data.accessToken; headers.Authorization=`Bearer ${accessToken}`; response=await fetch(`${API_BASE}${path}`,{...opts,headers}); }
  }
