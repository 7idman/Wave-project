          ...
          <div className="page-wrap">
            {page==="invest"&&<>
              ...
              <div id="auto-invest" className="gcard" style={{marginBottom:20,padding:mob?18:24}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr 1.2fr",gap:12,alignItems:"end"}}><div><div className="tlab">Contribution</div><div className="tiwrap" style={{marginBottom:0}}><input className="tinp" type="number" min="10" value={autoAmount} onChange={e=>setAutoAmount(e.target.value)}/><span className="tsfx">USD</span></div></div><div><div className="tlab">Frequency</div><select className="sel" value={autoFrequency} onChange={e=>setAutoFrequency(e.target.value)}><option>Weekly</option><option>Bi-weekly</option><option>Monthly</option></select></div><button className="btn btn-primary btn-lg" onClick={()=>toast2(`${autoTier} auto-invest plan saved: $${autoAmount || "0"} ${autoFrequency.toLowerCase()}.`)}>Activate plan</button></div>
              </div>
